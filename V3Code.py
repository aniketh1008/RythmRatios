...
def create_top_premium_table(parent_frame):
    """Create a table showing the highest net premium results for each ratio set"""
    global global_results

    # Create frame for the table
    table_frame = ttk.LabelFrame(parent_frame, text="Top Net Premium Results (All Ratio Sets)")
    ...
    tree.heading("net_premium", text="Net Premium", command=lambda: sort_table(tree, "net_premium", True))
    tree.heading("pnl", text="PNL", command=lambda: sort_table(tree, "pnl", True))
    ...
    if global_results:
        ...
        # Group by ratio and get top net premium for each
        ratio_groups = {}
        for trade in all_trades:
            ratio = trade["ratio"]
            if ratio not in ratio_groups or trade["net_premium"] > ratio_groups[ratio]["net_premium"]:
                ratio_groups[ratio] = trade

        # Add additional top trades for each ratio (not just the highest)
        top_trades_per_ratio = {}
        for trade in all_trades:
            ratio = trade["ratio"]
            if ratio not in top_trades_per_ratio:
                top_trades_per_ratio[ratio] = []
            top_trades_per_ratio[ratio].append(trade)

        # Sort each ratio's trades by net premium and get top 3
        all_top_trades = []
        for ratio, trades in top_trades_per_ratio.items():
            sorted_trades = sorted(trades, key=lambda x: x["net_premium"], reverse=True)
            all_top_trades.extend(sorted_trades[:3])  # Get top 3 trades per ratio

        # Sort all top trades by net premium
        sorted_top_trades = sorted(all_top_trades, key=lambda x: x["net_premium"], reverse=True)

        # Add to treeview
        for i, trade in enumerate(sorted_top_trades):
        ...
        # Sort initially by net premium
        sort_table(tree, "net_premium", True)

    return table_frame

def create_matrix_display(parent_frame):
    """
...
def create_application_ui(root):
    ...
    # Create matrix display in main frame
    create_matrix_display(main_frame)
...
def main():
    ...
    root.geometry("1200x900")  # Increased size to accommodate new table
