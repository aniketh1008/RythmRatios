...
def analyze_option_chain(option_chain_data, symbol_name="NIFTY", lot_size=50, strategy_params=None):
    """
...
def create_consolidated_display(parent_frame):
    """
    Create three separate tables for each offset value (from UI parameters)
    """
    global global_results
    ...
    if global_results:
        strategy_params = global_results[0].get("strategy_params", {})
    ...
    for offset in offset_values:
        create_offset_table(notebook, offset, organized_data)

    return main_display_frame

def organize_trade_data():
...
def create_offset_table(notebook, offset, organized_data):
    """Create a table for a specific offset value"""
...
def create_top_premium_table(parent_frame):
    """Create a table showing all negative net premium results sorted in ascending order"""
    global global_results
...
def create_application_ui(root):
    ...
    # Use the new consolidated display in the content frame
    create_consolidated_display(content_frame)

    # Create top premium table below matrix display
    create_top_premium_table(content_frame)
...
def fetch_and_analyze(root):
    ...
    try:
        ...
        for instrument in INSTRUMENTS:
            ...
            if option_chain:
                root.status_label.config(text=f"Analyzing {symbol_name}...")
                root.update()

                # Analyze option chain data with strategy parameters from UI
                analyze_option_chain(option_chain, symbol_name, lot_size, strategy_params)
            else:
